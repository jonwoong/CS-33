...
int find_it(int size) // find first fit block
{
    sizehdr = 16;
    int returnThis = -1;
    int i=0;
...
void lfree( int *aa )   // frees the block at address aa
{
    ...
    int payloadPosition = *aa;
    
    int B2header = (payloadPosition-sizehdr);
    int B2payload = hdr_payload(B2header);
    int B2footer = (B2header) + B2payload + sizehdr;
    
    int B1footer = (B2header-sizehdr);
    if (B1footer < 0) // Sometimes B1 footer comes out negative
    {
        B1footer = 0;
    }
    int B1payload = hdr_payload(B1footer);
    int B1header = (B1footer)-B1payload-sizehdr;
...
int lalloc( int amt )   // allocates a block of size int
{
    ...
    if (blockposition >= 0)
    {
        ...
        hdr_previus(blockposition) = blockposition-sizehdr-hdr_payload(blockposition-sizehdr)-sizehdr;
        if (blockposition == 0)
        {
        ...
        hdr_previus(blockposition+amt+sizehdr) = blockposition-sizehdr-hdr_payload(blockposition-sizehdr)-sizehdr;
        if (blockposition == 0)
        {
        ...
        hdr_previus(blockposition+amt+2*sizehdr) = blockposition;
